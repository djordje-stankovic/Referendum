...
class DbRepository extends AbstractRepository {
  ...
  async getEdits(id) {
    const { data, error } = await supabase.from('proposal_contributions').select('*').eq('proposal_id', id);
    if (error) throw new Error(error.message);
    return data;
  }
  ...
  async mergeEdit(proposalId, editId) {
    ...
    await supabase.from('proposal_contributions').update({ status: 'merged' }).eq('id', editId);
